...
Rectangle {
    ...
    function resize(){
        if(button.implicitWidth == 0){
            button.implicitWidth = 4 + _icon.width + _icon.anchors.leftMargin + _icon.anchors.rightMargin + label.width + label.anchors.leftMargin + label.anchors.rightMargin;
        }
        ...
        if(button.implicitHeight == 0){
            ...
            button.implicitHeight = Math.max(_iconHeight, labelHeight) + 4;
        }
